...
for epoch in range(opt.epoch, opt.n_epochs):
	for i, batch in enumerate(dataloader):
		...
		loss_G = ((loss_GAN_A2B + loss_GAN_B2A) * 0.5 + (loss_reg_A + loss_reg_B)) * (1. - rate) + (
					(loss_cycle_ABA + loss_cycle_BAB) * opt.lambda_cycle + (loss_pix_B + loss_pix_A) * opt.lambda_pixel) * rate
		...
		print(
			'loss_GAN_A2B: %.4f, loss_GAN_B2A: %.4f, loss_cycle_ABA: %.4f, loss_cycle_BAB: %.4f, loss_identity_A: %.4f, loss_identity_B: %.4f, loss_pix_A: %.4f, loss_pix_B: %.4f' % (
			loss_GAN_A2B.data[0],
			loss_GAN_B2A.data[0], loss_cycle_ABA.data[0], loss_cycle_BAB.data[0], loss_identity_A.data[0],
			loss_identity_B.data[0], loss_pix_A.data[0], loss_pix_B.data[0]))
